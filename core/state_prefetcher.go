...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.DiffStateDb, cfg vm.Config, interrupt *uint32) {
	var (
		header  = block.Header()
...
func precacheTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gaspool *GasPool, statedb *state.DiffStateDb, header *types.Header, tx *types.Transaction, cfg vm.Config) error {
